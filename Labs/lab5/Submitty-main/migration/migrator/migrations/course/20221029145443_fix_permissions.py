...
def up(config, database, semester, course):
    ...
    with(database_file).open('r') as db_file:
        ...
        database.execute(f"GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO {}"{db_info['database_course_user']})
        database.execute(f"GRANT SELECT, UPDATE ON ALL SEQUENCES IN SCHEMA public TO {}"{db_info['database_course_user']})
